...
function loadDB() {
  ...
  return {
    ...
    sessions: [],
    achievements: [
      { id: "first_play", title: "First Steps", description: "Play your first game", icon: "🎮" },
      { id: "score_100", title: "Century", description: "Score 100+ in any game", icon: "💯" },
      { id: "score_500", title: "High Flyer", description: "Score 500+ in any game", icon: "🚀" },
      { id: "top_10", title: "Leaderboard Legend", description: "Reach top 10 on any leaderboard", icon: "🏆" },
      { id: "play_5", title: "Committed", description: "Play 5+ times total", icon: "⭐" },
    ],
    player_achievements: [],
  };
}
...
function createScoreSDK(db, setDb, playerName, checkAchievements) {
  return {
    submitScore: (gameId, score) => {
      const newDb = { ...db };
      let player = newDb.players.find(p => p.name === playerName);
      if (!player) {
        player = { id: `p_${Date.now()}`, name: playerName, createdAt: new Date().toISOString() };
        newDb.players = [...newDb.players, player];
      }
      const entry = {
        id: `s_${Date.now()}`,
        playerId: player.id,
      ...
      newDb.scores = [...newDb.scores, entry];
      // increment plays
      newDb.games = newDb.games.map(g => g.id === gameId ? { ...g, plays: (g.plays || 0) + 1 } : g);
      saveDB(newDb);
      setDb(newDb);
      checkAchievements(newDb, player.id, gameId, score);
      return entry;
    },
  };
}
...
export default function GameHub() {
  ...
  const setDb = (newDb) => { saveDB(newDb); setDbRaw(newDb); };
  ...
  const registerGame = () => {
    let manifest;
    try {
      manifest = JSON.parse(aiManifest);
    } catch {
      manifest = {};
    }
    ...
    const newGame = {
      ...
      scoreType: manifest.scoreType || "highscore",
    ...
    const newDb = { ...db, games: [...db.games, newGame] };
    setDb(newDb);
    setAiManifest("");
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "system-ui, sans-serif" }}>
      ...
      <nav style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "12px 24px", display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <button onClick={() => setView("admin")} style={{ ...btnStyle(view === "admin" ? "primary" : "secondary"), fontSize: 12 }}>
          ⚙ Admin
        </button>
        {playerName && (
